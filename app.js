...
io.on("connection", (socket) => {
  ...
  socket.on("req_insert_issues", async (data) => {
    try {
      const result = await zone.insert_issues(data);
      socket.emit("return_req_insert_issues", result);
    } catch (err) {
      socket.emit("return_req_insert_issues", {
        status: 400,
        msg: err.message,
      });
    }
